...
read EnsID
grep $EnsID ./data/mRNA-ExonIDs.txt > mannual.txt
grep $EnsID ./data/Hs_Ensembl_exon.txt > exonlist.txt
echo -n "Enter another EnsID:"
read another
if [ $another != 0 ]
then
    grep $another ./data/mRNA-ExonIDs.txt > mannual1.txt
    grep $another ./data/Hs_Ensembl_exon.txt > exonlist1.txt
else
    echo "OK,that's it"
fi
